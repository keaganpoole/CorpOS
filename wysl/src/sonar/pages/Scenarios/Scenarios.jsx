...
export default function ScenariosPage() {
  ...
  const [scenarioPendingDelete, setScenarioPendingDelete] = useState(null);
  ...
  const openSelectionPanel = useCallback(
    (nodeId) => {
      ...
      // If this trigger has field criteria, show the criteria form.
  ...
  const handleSubOptionClick = (subOption) => {
    ...
    const triggerFieldDefinitions = (() => {
      ...
      if (subOption.key.startsWith('appointment_') && subOption.key !== 'appointment_soon') {
        return [
          { key: 'id', label: 'Record ID', type: 'text' },
          { key: 'person_id', label: 'Person ID', type: 'text' },
          { key: 'service_id', label: 'Service ID', type: 'text' },
          { key: 'staff_id', label: 'Staff ID', type: 'text' },
          ...getTableFields('appointments').filter((field) => field.key !== 'id'),
        ];
      }
      return [];
    })();
    ...
    if (panelCategory === 'TRIGGERS' && triggerFieldDefinitions.length > 0) {
      const initialTriggerConfig = {
        key: subOption.key,
        fields: Object.fromEntries(triggerFieldDefinitions.map((field) => [field.key, currentNode?.triggerConfig?.fields?.[field.key] || ''])),
      };
      setNodes(prev => prev.map(node => node.id === currentNodeId ? {
        ...node,
        configured: true,
                ...
                {panelStage === 'triggerConfig' && triggerConfig ? (
                  <div className="sb-action-config-form">
                    ...
                    <div className="sb-record-fields-grid">
                      {(() => {
                        const definitions = triggerConfig.key === 'incoming_call'
                          ? [{ key: 'phone_number', label: 'Phone Number', type: 'phone' }]
                          : triggerConfig.key === 'record_updated'
                            ? getRecordFieldsForTable(PEOPLE_RECORD_TABLE).filter((field) => field.key !== 'id')
        ...
        <div className="delete-confirm-overlay" onClick={handleCancelDelete}>
          <div className="delete-confirm-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="delete-confirm-message">
              Are you sure you want to delete "{scenarioPendingDelete?.name}"?
