...
const SelectEditor = ({ field, value, onChange, fieldConfig }) => {
  ...
  return (
    <div className="relative" ref={ref}>
      <button type="button" onClick={() => setOpen((next) => !next)} className={`flex min-h-[34px] w-full items-center justify-between gap-2 rounded-lg border px-2.5 py-1.5 text-left text-[11px] font-semibold tracking-[-0.02em] transition ${currentStyle.className}`}>
...
const LookupEditor = ({ value, options = [], onChange, placeholder = 'Search...' }) => {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div className="relative" ref={ref}>
      <button type="button" onClick={() => setOpen((next) => !next)} className="flex min-h-[34px] w-full items-center justify-between gap-2 rounded-lg border border-white/[0.06] bg-white/[0.025] px-2.5 py-1.5 text-left text-[11px] font-semibold tracking-[-0.02em] text-zinc-300 transition hover:border-white/15">
        <span className={`truncate ${current ? 'text-white' : 'text-zinc-700'}`}>{current?.label || 'Empty'}</span>
        <ChevronDown size={12} className="shrink-0 text-zinc-600" />
      </button>
      <AnimatePresence>
        {open && (
          ...
          >
            ...
            <div className="max-h-56 overflow-y-auto py-1">
              <button type="button" onClick={() => { onChange(null); setOpen(false); }} className="flex w-full items-center px-3 py-2 text-left text-[11px] font-semibold tracking-[-0.02em] text-zinc-500 hover:bg-white/[0.06]">
                Empty
              </button>
              {filtered.map((option) => (
...
const DateEditor = ({ value, onChange, type = 'date' }) => {
  const ref = useRef(null);
...
const FieldEditor = ({ field, value, onChange, errors, fieldConfig, lookupOptions }) => {
  ...
  if (!field.editable) {
    let display = value || '';
    if (field.type === 'timestamp') display = formatTimestampFull(value);
    if (field.type === 'date') display = formatDate(value);
    if (field.type === 'time' || field.type === 'computed_time') display = formatTime(value);
  ...
  if (field.type === 'person_lookup' || field.type === 'service_lookup' || field.type === 'receptionist_lookup') {
    return <LookupEditor value={value} options={lookupOptions[field.key] || []} onChange={onChange} placeholder={`Search ${field.label.toLowerCase()}...`} />;
  }
  ...
  if (field.type === 'date') return <DateEditor value={value} onChange={onChange} type="date" />;
  if (field.type === 'time') return <DateEditor value={value} onChange={onChange} type="time" />;
  if (field.type === 'textarea') return <textarea value={value || ''} onChange={(event) => onChange(event.target.value || null)} rows={4} className={`${baseClass} resize-none leading-relaxed`} />;
  if (field.type === 'timestamp') return <DateEditor value={value} onChange={onChange} type="datetime-local" />;
  return (
    <input
      type="text"
      value={value || ''}
...
const AppointmentDetailPanel = ({ appointment, onSave, onDelete, onClose, isNew = false, tableSchema = null, people = [], services = [], receptionists = [] }) => {
  ...
  const validate = () => {
    const errs = {};
    if (!currentAppointment.date) errs.date = 'Date is required';
    if (!currentAppointment.time) errs.time = 'Start time is required';
    setErrors(errs);
        ...
        <div className="shrink-0 border-b border-white/[0.04] px-7 py-6">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              <p className="mb-2 text-[11px] font-semibold tracking-[-0.02em] text-zinc-600">{isNew ? 'New Appointment' : 'Appointment'}</p>
