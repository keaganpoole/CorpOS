...
- **2026-04-17:** **ON-HIRE BUG FIXES.** `user_id` was missing from hired_receptionists insert (silent failures). `elevenlabs_voice_id` now correctly copied from catalog on hire. `refresh()` properly awaited after insert. Backend auto-restarted after changes.
