...
## System History
- **2026-04-30:** **NODE CONFIG `people_id` FIELDS + APPOINTMENT FORM REFACTOR.** Added `people_id` fields to: Create Payment, Create Appointment, Call Customer, Send SMS to Customer, Add Tag, Send Email. Added `record_id` to Update/Delete Record. Added `appointment_id` to Update/Delete Appointment. Refactored appointment config form from inline styles to CSS classes matching records form structure (`sb-record-field`, `sb-record-label`, `sb-input-field`). All text fields now have variable chip overlay support.
- **2026-04-29/30:** **VARIABLES PANEL SEARCH RECORDS + PREVIOUS NODE VARS.** Major VariablesPane rewrite. Added inline search per table section (magnifying glass icon triggers search input). Varbar animation on left edge during search. Matched field highlighting. Record navigation (up/down chevrons). Panel widened 20% (220→264px). Added "Current"/"Previous" section menu. Previous node variables traverse scenario graph backwards (BFS), showing variables from all prior nodes grouped by node, most recent first. Added Services variable set (orange). Condition modal: value field always plain text input, variable chip overlay only when `{{` present, variables panel follows dragged modal, condition modal spawns near top with 13% overlap. Condition modal styling cleanup (transparent backgrounds on close/remove buttons, no caret on key field, border removed from variable set groups).
- **2026-04-18:** **SONAR AGENT CUSTOMER CREATION FLOW.** Updated ElevenLabs agent workflow to automatically create people table records for new callers. The agent now: (1) Uses `identify_caller` to check if person exists, (2) If not found, calls `create_customer` to add them to people table, (3) For bookings, `create_appointment` auto-creates person if needed (linking via lead_id), (4) All calls log notes for future leads. Added `create_customer` tool definition in `setup-agent.js` and enhanced `create-customer` endpoint to accept/append notes. Updated agent system prompt with clear "NEW CUSTOMER RULES" section. **Backend restarted** after `better-sqlite3` recompiled for Node.js v24.14.0. **Root cause**: Node.js version mismatch (MODULE_VERSION 130 vs 137). Fixed by `npm rebuild better-sqlite3 --msvs_version=2022`. Sonar backend running on http://127.0.0.1:7878.
