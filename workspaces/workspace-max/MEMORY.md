...
## System History
- **2026-04-30:** **NODE CONFIG `person_id` FIELDS + APPOINTMENT FORM REFACTOR.** Added `person_id` fields to: Create Payment, Create Appointment, Call Customer, Send SMS to Customer, Add Tag, Send Email. Added `record_id` to Update/Delete Record. Added `appointment_id` to Update/Delete Appointment. Refactored appointment config form from inline styles to CSS classes matching records form structure (`sb-record-field`, `sb-record-label`, `sb-input-field`). All text fields now have variable chip overlay support.
