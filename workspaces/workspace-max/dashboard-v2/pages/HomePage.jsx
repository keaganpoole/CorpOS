...
import unifiedDashboardImage from '@/assets/passwordspage.png';
...
import AES256Stats from '../components/AES256Stats'; // Import the new component
...
const CyclingImageGrid = ({ allExpandedImages }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setIsMobile(mobile);
      // Update displayed images immediately on resize if count changes
  ...
  useEffect(() => {
    if (isInView && allExpandedImages.length > 0) {
      ...
      // Ensure initial images are set if not already
    ...
    >
      ...
      <div className="relative w-full max-w-xs h-full">
        <AnimatePresence initial={false}>
          {displayedImages[0] && (
            <motion.img
              key={displayedImages[0]} // Key for AnimatePresence
      ...
      {/* Slot 2 (only for desktop) */}
      {!isMobile && (
        <div className="relative w-full max-w-xs h-full">
          <AnimatePresence initial={false}>
            {displayedImages[1] && (
              <motion.img
                key={displayedImages[1]} // Key for AnimatePresence
...
const HomePage = () => {
  ...
  const LiquidForgeText = ({ text }) => {
    ...
    return (
      <span ref={ref} className={`anim-liquid-metal ${isInView ? 'animate' : ''}`} style={{ whiteSpace: 'pre' }}>
        {chars}
      </span>
    );
  };
  ...
  const RainingImages = () => {
    const [isInView, setIsInView] = useState(true); // Start immediately
    const ref = useRef(null);

    const imageVariants = {
      hidden: { y: -100, opacity: 0, rotate: 0 },
      visible: (i) => ({
        y: ["-10%", "110%"], // Fall from top to bottom, slightly off-screen
        x: [0, Math.random() * 400 - 200, 0], // Wider horizontal drift
        z: [Math.random() * -500, Math.random() * 500], // Random initial and final depth
        opacity: [0, 0.7, 0.7, 0], // Subtle fade in, stay, fade out
        scale: [0.5, 1.1, 1.1, 0.5], // Varying scale for depth effect
        rotateX: [0, Math.random() * 40 - 20, Math.random() * 40 - 20, 0], // More pronounced 3D rotation
        rotateY: [0, Math.random() * 40 - 20, Math.random() * 40 - 20, 0], // More pronounced 3D rotation
        rotateZ: [0, Math.random() * 30 - 15, Math.random() * 30 - 15, 0], // More pronounced 2D rotation
        filter: ['blur(10px)', 'blur(0px)', 'blur(0px)', 'blur(10px)'], // Dynamic blur for depth
        boxShadow: '0 4px 8px rgba(0, 0, 0, 0.2)', // Very slight drop shadow
        transition: {
          delay: i * 0.2 + Math.random() * 0.5, // Reduced stagger for quicker initial appearance
          duration: 12 + Math.random() * 10, // Random duration for each image
      ...
      >
        {isInView && cardImages.map((image, index) => (
          <motion.img
            ...
            style={{
              zIndex: 1,
              maxWidth: '320px', // Slightly larger max width for images
              maxHeight: '270px', // Slightly larger max height for images
              objectFit: 'contain', // Ensure entire image is visible
              pointerEvents: 'none', // Ensure images don't block interaction
              left: `${Math.random() * 120 - 10}%`, // Random horizontal start position, allowing off-screen left/right
              top: `-20%`, // Start off-screen at the top
  ...
  const CardDealerAnimation = ({ images }) => {
    const [isInView, setIsInView] = useState(false);
    const ref = useRef(null);

    const cardWidth = 350; // Define card width
    const cardHeight = 250; // Define card height
    const cardSpacing = 20; // Spacing between cards

    const cardVariants = {
      hidden: { x: -cardWidth * 2, y: -50, opacity: 0, rotateY: 120, rotateZ: -30, scale: 0.7, filter: 'blur(5px)', boxShadow: '0 0 0 rgba(0,0,0,0)' },
      visible: (i) => ({
        x: 0, // Final X position (relative to its flex position)
        ...
        scale: 1,
        filter: 'blur(0px)',
        boxShadow: '0 5px 15px rgba(0,0,0,0.2)', // Final shadow
        transition: {
          delay: i * 0.4, // Reduced stagger for quicker initial appearance
          duration: 1.0, // Longer duration for smoother deal
          ease: "easeOut",
          type: "spring",
          stiffness: 120,
          damping: 15,
      ...
      dust: { opacity: [0, 0.5, 0], scale: [0.5, 1, 1.5], filter: ['blur(0px)', 'blur(2px)', 'blur(0px)'], transition: { delay: i * 0.7 + 0.6, duration: 0.4, ease: "easeOut" } },
    };

    return (
      <motion.div
        ref={ref}
        className="relative flex justify-center items-center h-[400px] mt-10 mx-auto space-x-[20px]" // Use space-x for spacing
        onViewportEnter={() => setIsInView(true)}
        onViewportLeave={() => setIsInView(false)}
        viewport={{ once: true, amount: 0.5 }}
        style={{ perspective: 1000 }}
      >
        {isInView && images.map((image, index) => (
          <motion.div
            key={index}
            className="rounded-lg overflow-hidden shadow-lg"
            style={{
              width: `${cardWidth}px`, // Card width
              height: `${cardHeight}px`, // Card height
              zIndex: index,
            ...
            <AnimatePresence>
              {isInView && (
                <motion.div
                  ...
                  animate={{ opacity: [0, 0.3, 0], scale: [0.8, 1.2], transition: { delay: index * 0.7 + 0.6, duration: 0.4 } }}
                  exit={{ opacity: 0 }}
                >
                  <div className="w-full h-full bg-gradient-to-br from-gray-200 to-gray-50 opacity-0 rounded-full"
                       style={{ filter: 'blur(10px)', transform: 'scale(0.1)' }}></div>
                </motion.div>
              )}
            </AnimatePresence>
          </motion.div>
      ...
      const encryptionTextRef = useRef(null);

      const isEncryptionInView = useInView(encryptionTextRef, { once: true, amount: 0.5 });

      const [isGlitching, setIsGlitching] = useState(false);

    

      const applyGlitchAnimation = (text) => {
        ...
        const targetEl = encryptionTextRef.current;

        if (!targetEl) return;

    

        targetEl.innerHTML = ''; // Clear previous text
        ...
        text.split('').forEach((char, index) => {
          ...
          span.style.setProperty('--char', `'${char}'`);
        ...
        // For animations that need element width (though glitch doesn't directly use it, good practice)
        ...
        // Add the 'animating' and 'glitch' classes

        targetEl.classList.add('animating', 'glitch');

    

            // The animation's `animation-fill-mode: forwards` in CSS will keep the final state.
      ...
      useEffect(() => {
        ...
        if (isEncryptionInView && encryptionTextRef.current) {

          applyGlitchAnimation("AES-256");
  ...
  const AnimatedBeautifulText = () => {
    ...
    useEffect(() => {
      if (isInView) {
        setAnimationKey(prevKey => prevKey + 1); // Trigger animation when in view
      }
    }, [isInView]);

    const getDelay = (index) => index * 50; // For prism-1 (Adjusted for faster animation)
    ...
    <div id="myHtmlContent">
      ...
      <main>
        <section className="content-section light-bg text-center relative overflow-hidden">
          <h3 data-animate="true">One <AnimatedBeautifulText /> Passwords Manager</h3>
          <p data-animate="true">
            Forget the sticky notes and messy notebook. Keyquarters organizes all your passwords, logins, and notes into one beautiful, secure vault, protected by a powerful encryption algorithm used by top banks and government agencies for classified data.
          </p>
          <RainingImages />
        </section>
        ...
        <section className="content-section dark-bg text-center">
          <h3 data-animate="true">No More Account <span className="glitch-text">Lockouts</span></h3>
          <p data-animate="true">
            Access all your passwords with just one 4-digit passcode. Your private vault is beautifully organized, intelligently sorted, and personalized — always secure, never stored anywhere, and accessible from any device.
          </p>
        ...
        <section className="content-section dark-bg text-center">
          ...
          <p data-animate="true">
            Lightning-fast, AI-powered search means you’ll never waste time hunting or updating a password again. Keyquarters finds what you need the moment you need it—anywhere in your vault.
          </p>
        ...
        <section className="hero-section text-center">
          ...
          <p className="hero-subheadline" data-animate="true">
            Meet Breezy — your personal device and login companion.
            She knows your device inside and out, helping you with login issues, account access, and anything specific to your phone. Ask about errors or hidden settings, and she’ll guide you naturally. Over time, Breezy learns your preferences, making it feel more like chatting with a friend who just gets it.
          </p>
        ...
        {/* Phone Helper Section */}
        <section className="content-section light-bg text-center flex flex-col justify-center items-center">
          <h3 data-animate="true"><span ref={encryptionTextRef} className="animated-text">AES-256</span><span className="static-text">Encryption</span></h3>
          <p data-animate="true">
            Keyquarters guards your passwords with the highest security, using the same standards trusted by top banks and government agencies. Your actual passwords are never stored on our servers, keeping it completely private. Even in the unlikely event of a breach, your passwords remain unreadable, giving you total peace of mind.
          </p>
          <AES256Stats startAnimation={isEncryptionInView} />

        </section>
      </main>
      <footer className="bg-black text-gray-400 py-8 text-center">
        <div className="container mx-auto px-4">
          <p className="text-sm">&copy; 2025 Keyquarters. All rights reserved.</p>
