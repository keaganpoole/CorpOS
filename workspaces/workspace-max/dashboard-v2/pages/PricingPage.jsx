...
import { useAuth } from '../contexts/AuthContext'; // Using the context to get user data
...
const planHierarchy = { "Free": 0, "Unlimited": 1, "Unlimited Pro": 2 };

const plansConfig = [
    {
        name: "Unlimited Pro",
        description: "Unlock Breezy — the AI assistant that understands your device, adapts to your habits, and solves login and device issues with ease.",
        prices: { // These are the display prices for each source
            Standard: { monthly: 9, annually: 7 }, // $84/yr
            Sales: { monthly: 49, annually: 41 }, // $492/yr
            Social: { monthly: 9, annually: 7 }, // $84/yr
        },
        features: ["Everything in Unlimited", "Device & Login AI Assistant"],
        isRecommended: true
    },
    {
        name: "Unlimited",
        description: "Store unlimited passwords and other digital records at the highest security with AES-256 encryption.",
        prices: { // These are the display prices for each source
            Standard: { monthly: 6, annually: 5 }, // $60/yr
            Sales: { monthly: 29, annually: 25 }, // $300/yr
            Social: { monthly: 6, annually: 5 }, // $60/yr
        },
        features: ["Store unlimited passwords", "Access on all your devices", "24/7 domestic customer support"],
        isRecommended: false
    },
    {
        name: "Free",
        description: "For individuals who are just getting started.",
        prices: { // Free plan has no different pricing based on source
            Standard: { monthly: 0, annually: 0 },
            Sales: { monthly: 0, annually: 0 },
            Social: { monthly: 0, annually: 0 }
        },
        features: ["Limited use of all features"],
        isRecommended: false
    },
];
...
const PlanCard = ({ plan, cycle, isInitialLoad, index, currentUserPlan, hasStartedTrial, isTestMode }) => {
    ...
    const handleUpgradeClick = async () => {
        ...
        console.log("Attempting checkout with:", { priceId, session });
        try {
            ...
            console.log("Full Axios response from create-checkout-session:", response);
    ...
    useEffect(() => {
        ...
        savingsEl.innerHTML = '\u00A0'; // Clear savings text
        savingsEl.classList.remove('animate', 'savings-gradient-text', 'text-green-500');
        
        // Always animate the price
        if (price > 0) {
            scrambleFx.current.setText(String(price)).then(() => {
                // If annually, show the savings message after the animation
            ...
            // For the free plan, just set the text without animation
...
const PricingPage = () => {
    ...
    // Fetch TEST_MODE status from backend
    useEffect(() => {
        const fetchConfig = async () => {
            try {
                ...
                // Keep isTestMode as false on error
            }
        };
    ...
    // Handle Source Cookie & URL Parameter
    useEffect(() => {
    ...
    const plansToDisplay = plansConfig.map(plan => {
        ...
        // If data hasn't loaded yet, return empty/loading state for paid plans
        ...
        if (!product) return { ...plan, price: { monthly: 0, annually: 0 }, priceIds: {} };
        ...
        return {
            ...plan,
            price: {
                monthly: monthlyPriceObj ? monthlyPriceObj.unit_amount / 100 : 0,
                annually: annualPriceObj ? annualPriceObj.unit_amount / 100 : 0
            },
            priceIds: {
    ...
    return (
        <div className="pricing-page-bg text-gray-300 antialiased min-h-screen">
            <svg width="0" height="0" style={{ position: 'absolute' }}><defs>
                <linearGradient id="checkGradient" x1="0%" y1="0%" x2="100%" y2="0%"><stop offset="0%" stopColor="#ef8f20" /><stop offset="100%" stopColor="#ba5914" /></linearGradient>
            </defs></svg>
            ...
            <div className="container mx-auto px-3 py-12 sm:py-20"> 
                <div className="text-center max-w-3xl mx-auto mb-12">
                    ...
                    <p className="text-lg text-gray-400">Choose a plan that's right for you.</p>
                </div>
                ...
                <div className="relative text-center">
                    <div className="inline-grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 py-4">
