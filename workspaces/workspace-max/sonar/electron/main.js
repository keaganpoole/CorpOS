...
async function startBackend() {
  ...
  try {
    ...
    console.log(`[Sonar] Backend ready on port ${BACKEND_PORT}`);
  } catch (err) {
    console.error('[Sonar] Backend failed to start:', err.message);
  }
}
...
ipcMain.handle('control:runtime', async (event, { mode }) => {
  ...
  controller.events.emit({
    ...
    message: `Runtime ${mode} from SONAR`,
    actor: 'user',
    actor_type: 'user',
    source: 'sonar_ui',
  });
