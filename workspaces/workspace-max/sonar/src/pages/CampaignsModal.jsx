...
import {
  ...
  Pencil, Sparkles, Globe, Crosshair, XCircle,
} from 'lucide-react';
...
const STATUS_CONFIG = {
  Unassigned: { color: 'text-zinc-500',   bg: 'bg-zinc-500/10',    border: 'border-zinc-500/20',    glow: '',                                        icon: <Hash size={10} /> },
  Assigned:   { color: 'text-indigo-400', bg: 'bg-indigo-500/10',  border: 'border-indigo-500/20',  glow: 'shadow-[0_0_12px_rgba(129,140,248,0.08)]', icon: <Target size={10} /> },
  ...
  Completed:  { color: 'text-emerald-400',bg: 'bg-emerald-500/10', border: 'border-emerald-500/20', glow: 'shadow-[0_0_12px_rgba(52,211,153,0.08)]',  icon: <Check size={10} /> },
  Failed:     { color: 'text-rose-400',   bg: 'bg-rose-500/10',    border: 'border-rose-500/20',    glow: 'shadow-[0_0_12px_rgba(244,63,94,0.08)]',   icon: <XCircle size={10} /> },
};
const STATUS_OPTIONS = ['Unassigned', 'Assigned', 'Active', 'Completed', 'Failed'];

// ─── Campaign Card ─────────────────────────────────────────────────────────
const CampaignCard = ({ campaign, onUpdate, onDelete, isSelected, onAssign, onUnassign, agent }) => {
  const [editing, setEditing] = useState(false);
  const [form, setForm] = useState({});
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);

  const statusConf = STATUS_CONFIG[campaign?.Status] || STATUS_CONFIG.Unassigned;

  const startEditing = () => {
    setForm({
      'Campaign Name': campaign?.['Campaign Name'] || '',
      'Status': campaign?.Status || 'Unassigned',
  ...
  if (editing) {
    return (
      <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
        className="bg-[#0d0d0f] border border-cyan-500/15 rounded-2xl overflow-hidden">
        <div className="px-5 pt-4 pb-3 flex items-center justify-between border-b border-white/[0.04]">
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg bg-cyan-500/10 border border-cyan-500/15"><Pencil size={11} className="text-cyan-400" /></div>
            <span className="text-[10px] font-black text-cyan-400 uppercase tracking-[0.15em]">Editing</span>
          </div>
          <button onClick={() => setEditing(false)} className="p-1.5 rounded-lg text-zinc-600 hover:text-white hover:bg-white/5 transition-all"><X size={12} /></button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            ...
            <input type="text" value={form['Campaign Name']} onChange={e => setForm({ ...form, 'Campaign Name': e.target.value })}
              className="w-full bg-black/50 border border-white/[0.06] rounded-xl px-4 py-2.5 text-[12px] text-white font-semibold focus:outline-none focus:border-cyan-500/30 transition-colors placeholder:text-zinc-800" autoFocus />
          </div>
          <div>
        ...
        <div className="px-5 py-3 border-t border-white/[0.04] flex items-center gap-2">
          ...
          <button onClick={() => setEditing(false)} className="px-4 py-2 rounded-xl text-[9px] font-bold uppercase tracking-wider text-zinc-500 hover:text-white transition-all">Cancel</button>
      ...
      <div className="p-5">
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1 min-w-0 mr-3">
            ...
            <div className="flex items-center gap-2 mt-2">
              <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[8px] font-black uppercase tracking-widest ${statusConf.bg} ${statusConf.color} border ${statusConf.border} ${statusConf.glow}`}>
                {statusConf.icon} {campaign?.Status || 'Unassigned'}
              </span>
              {campaign?.assigned_to && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[8px] font-bold text-indigo-400 bg-indigo-500/10 border border-indigo-500/15">
                  → {campaign.assigned_to}
                </span>
              )}
              {campaign?.['Target Industry'] && (
                <span className="inline-flex items-center gap-1 text-[10px] text-zinc-500"><Building2 size={10} /> {campaign['Target Industry']}</span>
              )}
            </div>
          </div>
          <div className="flex items-center gap-1.5 shrink-0">
            {agent && (
              isSelected ? (
                <button onClick={(e) => { e.stopPropagation(); onUnassign(campaign); }}
                  className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[8px] font-bold uppercase tracking-wider hover:bg-rose-500/10 hover:border-rose-500/20 hover:text-rose-400 transition-all">
                  <X size={10} /> Unassign from {agent.name}
                </button>
              ) : (
                (!campaign?.assigned_to || campaign?.Status === 'Unassigned') && (
                  <button onClick={(e) => { e.stopPropagation(); onAssign(campaign); }}
                    className="opacity-0 group-hover:opacity-100 flex items-center gap-1 px-2.5 py-1.5 rounded-xl bg-white/[0.03] border border-white/[0.06] text-zinc-500 text-[8px] font-bold uppercase tracking-wider hover:text-white hover:bg-white/[0.06] transition-all">
                    <Target size={10} /> Assign to {agent.name}
                  </button>
                )
              )
            )}
            <button onClick={startEditing}
              className="opacity-0 group-hover:opacity-100 transition-all p-2 rounded-xl bg-white/[0.03] border border-white/[0.06] text-zinc-500 hover:text-white hover:bg-white/[0.06]">
              <Pencil size={12} />
            </button>
          </div>
        </div>
        <div className="grid grid-cols-3 gap-3 pt-3 border-t border-white/[0.03]">
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg bg-indigo-500/[0.06] border border-indigo-500/10"><Target size={10} className="text-indigo-400/70" /></div>
            <div><p className="text-[7px] text-zinc-700 font-bold uppercase tracking-widest">Goal</p><p className="text-[12px] font-bold text-zinc-300">{campaign?.['Lead Count Goal'] || '—'}</p></div>
          </div>
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg bg-cyan-500/[0.06] border border-cyan-500/10"><MapPin size={10} className="text-cyan-400/70" /></div>
            <div><p className="text-[7px] text-zinc-700 font-bold uppercase tracking-widest">States</p><p className="text-[12px] font-bold text-zinc-300 truncate">{toArray(campaign?.['Target State(s)']).join(', ') || '—'}</p></div>
          </div>
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg bg-amber-500/[0.06] border border-amber-500/10"><Globe size={10} className="text-amber-400/70" /></div>
            <div><p className="text-[7px] text-zinc-700 font-bold uppercase tracking-widest">Cities</p><p className="text-[12px] font-bold text-zinc-300 truncate">{toArray(campaign?.['Target City(s)']).join(', ') || '—'}</p></div>
          </div>
        </div>
      </div>
    </motion.div>
...
const NewCampaignForm = ({ onCreate, onCancel }) => {
  const [form, setForm] = useState({
    'Campaign Name': '', 'Status': 'Unassigned', 'Target Industry': '',
  ...
  const handleCreate = async () => {
    ...
    try {
      await onCreate({
        'Campaign Name': form['Campaign Name'], 'Status': form['Status'], 'Target Industry': form['Target Industry'],
        'Target State(s)': form['Target State(s)'].split(',').map(s => s.trim()).filter(Boolean).join(', '),
        'Target City(s)': form['Target City(s)'].split(',').map(s => s.trim()).filter(Boolean).join(', '),
        'Lead Count Goal': parseInt(form['Lead Count Goal']) || null,
      });
  ...
  return (
    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
      <div className="bg-[#0d0d0f] border border-indigo-500/15 rounded-2xl overflow-hidden mb-4">
        <div className="px-5 pt-4 pb-3 flex items-center justify-between border-b border-white/[0.04]">
          <div className="flex items-center gap-2">
            <div className="p-1.5 rounded-lg bg-indigo-500/10 border border-indigo-500/15"><Sparkles size={11} className="text-indigo-400" /></div>
            <span className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.15em]">New Campaign</span>
          </div>
          <button onClick={onCancel} className="p-1.5 rounded-lg text-zinc-600 hover:text-white hover:bg-white/5 transition-all"><X size={12} /></button>
        </div>
        <div className="p-5 space-y-4">
          <div>
            <label className="text-[8px] font-black text-zinc-600 uppercase tracking-[0.2em] mb-1.5 block">Campaign Name *</label>
          ...
          <button onClick={handleCreate} disabled={creating || !form['Campaign Name']}
            className={`w-full py-3 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all active:scale-95 ${
              !form['Campaign Name'] ? 'bg-white/[0.03] text-zinc-700 cursor-not-allowed border border-white/[0.04]' : 'bg-white text-black hover:bg-indigo-400'
            }`}>
            {creating ? <Loader2 size={11} className="inline animate-spin mr-1.5" /> : <Plus size={11} className="inline mr-1.5" />}Create Campaign
          </button>
        </div>
      </div>
    </motion.div>
...
// ─── Main Modal ────────────────────────────────────────────────────────────
const CampaignsModal = ({ agent, onClose, onCampaignAssigned }) => {
  const [campaigns, setCampaigns] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [statusFilter, setStatusFilter] = useState('All');
  const [showNew, setShowNew] = useState(false);
  // Find which campaign is assigned to this agent (updates after load)
  const [selectedId, setSelectedId] = useState(null);

  useEffect(() => {
    if (campaigns.length > 0 && agent) {
      const assigned = campaigns.find(c => c.assigned_to?.toLowerCase() === agent.name?.toLowerCase());
      setSelectedId(assigned?.id || null);
    }
  }, [campaigns, agent]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const { data, error: err } = await supabase.from('research_campaigns').select('*').order('created_at', { ascending: false });
        if (err) throw err;
  ...
  const handleCreate = async (d) => {
    const { data, error: err } = await supabase.from('research_campaigns').insert(d).select().single();
    if (err) throw err;
    setCampaigns(prev => [data, ...prev]);
  };
  const handleUpdate = async (id, u) => {
    const { data, error: err } = await supabase.from('research_campaigns').update(u).eq('id', id).select().single();
    if (err) throw err;
  ...
  const handleAssign = async (campaign) => {
    if (!agent) return;
    setSelectedId(campaign.id);
    const agentName = agent.name;
    setCampaigns(prev => prev.map(c => c.id === campaign.id ? { ...c, Status: 'Assigned', assigned_to: agentName } : c));
    try {
  ...
  const handleUnassign = async (campaign) => {
    if (!agent) return;
    setSelectedId(null);
    setCampaigns(prev => prev.map(c => c.id === campaign.id ? { ...c, Status: 'Unassigned', assigned_to: null } : c));
    try {
  ...
  const statusCounts = STATUS_OPTIONS.reduce((a, s) => { a[s] = campaigns.filter(c => c?.Status === s).length; return a; }, {});
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      <motion.div
        ...
        className="relative w-[680px] max-h-[720px] bg-[#08080a] border border-white/[0.06] rounded-3xl flex flex-col overflow-hidden shadow-[0_40px_100px_rgba(0,0,0,0.95)]">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[8px] text-zinc-600 font-bold uppercase tracking-widest mt-0.5">
                  {campaigns.length} total · {statusCounts.Assigned || 0} assigned · {statusCounts.Active || 0} active
                </p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              <button onClick={() => setShowNew(!showNew)}
                className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-wider transition-all active:scale-95 ${
                  showNew ? 'bg-rose-500/10 border border-rose-500/20 text-rose-400 hover:bg-rose-500/15' : 'bg-white text-black hover:bg-indigo-400 hover:text-white'
                }`}>
                {showNew ? <X size={10} /> : <Plus size={10} />}
                {showNew ? 'Cancel' : 'New Campaign'}
              </button>
              <button onClick={onClose} className="p-2 rounded-xl text-zinc-600 hover:text-white hover:bg-white/[0.04] transition-all"><X size={14} /></button>
            </div>
          </div>
              ...
              return (
                <button key={s} onClick={() => setStatusFilter(s)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[9px] font-bold uppercase tracking-wider transition-all ${
                    isActive ? `${conf ? `${conf.bg} ${conf.color} border ${conf.border}` : 'bg-white/[0.06] text-white border border-white/[0.08]'}` : 'text-zinc-600 hover:text-zinc-400 hover:bg-white/[0.02]'
                  }`}>
                  {conf && conf.icon} {s}
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-3">
          <AnimatePresence>{showNew && <NewCampaignForm key="new" onCreate={handleCreate} onCancel={() => setShowNew(false)} />}</AnimatePresence>
          {loading && (
          ...
          {!loading && !error && filtered.map(c => (
            <CampaignCard key={c.id} campaign={c} onUpdate={handleUpdate} onDelete={handleDelete}
              isSelected={selectedId === c.id} onAssign={handleAssign} onUnassign={handleUnassign} agent={agent} />
          ))}
          {!loading && !error && filtered.length === 0 && !showNew && (
            <div className="flex flex-col items-center justify-center py-20">
              <div className="p-4 rounded-2xl bg-white/[0.02] border border-white/[0.04] mb-4"><Target size={28} className="text-zinc-700" /></div>
              ...
              <p className="text-[9px] text-zinc-700">Create one to start targeting Relics</p>
            </div>
          )}
        </div>
      </motion.div>
